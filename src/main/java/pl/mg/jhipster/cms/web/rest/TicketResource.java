...
import pl.mg.jhipster.cms.repository.search.TicketSearchRepository;
...
/**
 * REST controller for managing {@link pl.mg.jhipster.cms.domain.Ticket}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TicketResource {
    ...
    public ResponseEntity<List<Ticket>> searchTickets(
        ...
        Page<Ticket> page = ticketSearchRepository.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
